export function setPath<T>(object: T, path: string, defaultValue: any) {
  return _setPathNormalized(object, _stringToPathArray(path), defaultValue)
}
...
const _stringToPathArray = <T extends string>(path: T): (string | number)[] => {
  if (path.length === 0) return []

  const match = path.match(/^\[(.+?)\](.*)$/) || path.match(/^\.?([^\.\[\]]+)(.*)$/)
  if (match) {
    const [_, key, rest] = match
    return [/^\d+$/.test(key) ? Number(key) : key, ..._stringToPathArray(rest)]
  }
  return [path]
}
